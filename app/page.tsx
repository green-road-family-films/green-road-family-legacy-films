...
export default function Home(){
 ...
 return <main>
  ...
  <section id="stories" className="stories section-pad"><div className="section-head"><div><span className="section-number">005 / STORIES</span><h2>What deserves<br/><em>to be carried forward?</em></h2></div></div><div className="story-territories">{stories.map((story,index)=><article key={story}><span>0{index+1}</span><h3>{story}</h3><button aria-label={`Explore ${story}`}>↗</button></article>)}</div></section>
  ...
  <section id="faq" className="faq section-pad"><div className="faq-title"><span className="section-number">016 / FAQ</span><h2>Before we<br/><em>begin.</em></h2></div><div className="faq-list">{faq.map((item,index)=><div className={open===index?"faq-item open":"faq-item"} key={item[0]}><button onClick={()=>setOpen(open===index?null:index)} aria-expanded={open===index}><span>{String(index+1).padStart(2,"0")}</span><b>{item[0]}</b><i>{open===index?"−":"+"}</i></button><div><p>{item[1]}</p></div></div>)}</div></section>
