...
export default function Home(){
 ...
 return <main onMouseMove={move}>
  ...
  <section className="final-cta section-pad"><div className="final-cta-layout"><header className="final-cta-heading"><p className="kicker gold">{final.lines[0]}</p><h2>{final.lines[1]}</h2><a className="button light" href="#inquire">Begin a private conversation <Arrow/></a></header><div className="final-cta-copy"><div className="final-cta-opening"><Copy lines={final.lines.slice(2,5)}/></div><div className="final-cta-reassurance"><Copy lines={final.lines.slice(5,9)}/></div><div className="final-cta-values">{final.lines.slice(9,12).map(line=><span key={line}>{line}</span>)}</div><div className="final-cta-closing"><Copy lines={final.lines.slice(12,final.lines.indexOf("[ BEGIN A PRIVATE CONVERSATION ]"))}/></div></div></div></section>
